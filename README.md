...
C'est la méthode la plus simple et la plus rapide. Aucune installation de Java, Node.js ou MySQL n'est nécessaire. Il suffit d'avoir **Docker** installé.
...
### 2. Vérifier que les conteneurs fonctionnent
```bash
docker compose ps
```
Les trois conteneurs suivants doivent être en état **running** :
- `business_analysis_db` — Base de données MySQL
- `business_analysis_backend` — API Spring Boot (port 8080)
- `business_analysis_frontend` — Application Angular (port 4200)
...
| Swagger (Documentation API) | [http://localhost:8080/swagger-ui/index.html](http://localhost:8080/swagger-ui/index.html) |
...
Si vous préférez lancer le projet sans Docker, suivez les instructions ci-dessous.
...
npm start
```
*(La commande `npm start` lance `ng serve` en arrière-plan)*
...
- **Documentation API (Swagger UI)** : [http://localhost:8080/swagger-ui/index.html](http://localhost:8080/swagger-ui/index.html)
